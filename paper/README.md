...
The industrial execution sequence is now explicit in `goals/`: Gate 0 qualifies whether each local dataset can test C1/C2/C3; Gate 1 executes the final learned path through PHMFactory; Gate 2 runs one target × one seed across all decisive cells; only then is the frozen multi-fold/multi-seed study authorized. `goals/07_LOCAL_AGENT_EXECUTION.md` is the copyable local execution prompt.
