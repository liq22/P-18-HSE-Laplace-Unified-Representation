...
The paired tuple $(C_F,z_o,w_0)$ makes the required joint reference law observable on the source design. It does not guarantee accurate estimation or validity on a new machine. Only blocks jointly observed in a source reference design, or identified under a stated alternative observation model, are admitted together. A union of separately available blocks is not a substitute. Deployment descriptors determine which source-defined target applies; target labels never select it. A different complete condition requires its corresponding identification argument. Source-to-target conditional transport and overlap are the assumptions in Section 2.3, not consequences of pairing. Held-out datasets probe their consequences; they do not prove population identification.

**Constructing the eligible basis.** Choose one source-justified joint block with orthonormal basis $Q_e$, whose joint conditional with the full visible record is identified. For a calibrated linear acquisition, compute a full singular-value decomposition

$$
A_e Q_e=U\Sigma V^\top,\qquad
B_e=Q_e V_{\rm null},\qquad G_e=B_eB_e^\top.
$$

$V_{\rm null}$ contains the right-singular vectors in the declared nullspace, so $\operatorname{range}(B_e)=\operatorname{range}(Q_e)\cap\ker(A_e)$. Choose the metric and numerical tolerance on normalized source operators; report sensitivity for weak directions. SVD supplies geometry after joint qualification, not an identification test. Distinct marginal blocks are never combined merely because each appears in some source. An empty intersection invokes no missing-state model. If the calibrated observation relation is unavailable, specify a reference-feature acquisition explicitly rather than assigning physical support to an arbitrary latent index.
...
This is an inherited temporal bias from the published LLapDiff model [@you2026llapdiff], not Laplace-distributed perturbation. Laplace structure acts on physical query time $\widetilde t$; it does not replace the diffusion-level embedding $k$ or alter the Gaussian forward law. The published name LLapDiff is retained for that inherited model, while the proposed adaptation is described as support-constrained, Laplace-structured latent diffusion. The modal branch is embedded in the denoiser, whose output in Eq. (9) is velocity. Therefore a decaying basis does not prove that the clean generated sample, the nonlinear denoiser or the reverse process is globally stable. The ordinary temporal comparator replaces this branch under the same condition, target, prediction type and selection budget. Physical modal interpretation additionally requires the calibrated reference; learned poles alone do not supply it.
...
### 3.4 Reverse update, conditional uncertainty coupling and diagnosis
...
Measured values are not clamped as clean latents. An observed-state readout $q_o(z_o\mid C_F)$ is fitted on the same source reference pairs, or calculated analytically under a known observation/noise model. The first learned readout may be Gaussian and is evaluated as an approximation. It is shared across mechanism arms. For an exact measured-reference feature acquisition with no added observation noise, $q_o$ is instead a point mass at that measured feature; this is not a noise-free physical-state assertion. The conditional sampling construction is
...
Each $z_o^{(b)}$ remains fixed within its reverse trajectory. The construction retains a conditional dependence path; matching the reference joint law remains an empirical question. Independently drawing the observed and missing marginal posteriors is a different, generally incoherent procedure. In the primary mechanism comparison, a common diagnostic readout $h_\psi$ is trained on source reference tuples and source labels, then frozen. It retains the same observed code $R$ and averages conditional predictions:
...
| A2 | Select one jointly identified source block under the complete observation; construct $B_e=Q_eV_{\rm null}$ from the source-calibrated operator and freeze $G_e$ and the deployment descriptor rule. |
