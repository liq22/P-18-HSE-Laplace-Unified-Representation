...
8. **Cache models locally**: Set `HF_HOME` for all Hugging Face caches (Hub cache at `$HF_HOME/hub`), or `HF_HUB_CACHE` just for Hub files; use `HF_HUB_OFFLINE=1` only after the required pinned model snapshots are already cached, to enforce the revision recorded in `paper/experiments/reproducibility.md`
